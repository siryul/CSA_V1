...
def mixup_criterion_2(criterion, pred, y_a, y_b, lam, current_epoch, total_epoch):
  return lam * criterion(pred, y_a, current_epoch, total_epoch) + (1-lam) * criterion(
    pred, y_b, current_epoch, total_epoch)
...
class LabelAwareSmoothing(nn.Module):
  ...
  def __init__(self, cls_num_list, smooth_head, smooth_tail, shape='concave', power=None):
    ...
    if shape == 'concave':
      ...
      self.smooth = smooth_tail + (smooth_head-smooth_tail) * (np.array(cls_num_list) - n_K) / (n_1-
                                                                                                n_K)

    elif shape == 'convex':
      self.smooth = smooth_head + (smooth_head-
                                   smooth_tail) * np.sin(1.5 * np.pi +
                                                         (np.array(cls_num_list) - n_K) * np.pi /
                                                         (2 * (n_1-n_K)))
